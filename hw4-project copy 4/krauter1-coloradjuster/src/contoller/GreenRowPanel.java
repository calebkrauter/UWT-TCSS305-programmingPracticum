...
public class GreenRowPanel extends JPanel implements PropertyChangeListener {
    ...
    private void addListeners() {
        final int initialValue = myColor.getGreen();
        ...
        myDecreaseButton.addActionListener(theEvent -> {
            if (myColor.getGreen() == minValue) {
                myDecreaseButton.setEnabled(false);
            } else {
                myColor.adjustGreen(-1);
            }
            if (myColor.getGreen() < maxValue && !myIncreaseButton.isEnabled()) {
                myIncreaseButton.setEnabled(true);
            }
        });
        myIncreaseButton.addActionListener(theEvent -> {
            if (myColor.getGreen() == maxValue) {
                myIncreaseButton.setEnabled(false);
            } else {
                myColor.adjustGreen(1);
            }
            if (myColor.getGreen() > minValue && !myDecreaseButton.isEnabled()) {
        ...
        myValueSlider.addChangeListener(theEvent -> {
            ...
            if (myColor.getGreen() > minValue && !myDecreaseButton.isEnabled()) {
                myDecreaseButton.setEnabled(true);
            } else if (myColor.getGreen() == minValue) {
                myDecreaseButton.setEnabled(false);
            }
            if (myColor.getGreen() < maxValue && !myIncreaseButton.isEnabled()) {
                myIncreaseButton.setEnabled(true);
            } else if (myColor.getGreen() == maxValue) {
                myIncreaseButton.setEnabled(false);
            }
        });
        ...
        myValueField.addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(final ActionEvent theEvent) {
                try {
                    if (myColor.getGreen() >= minValue && myColor.getGreen() <= maxValue) {
                        myColor.setGreen(Integer.valueOf(parseInt(myValueField.getText())));
                    }
                } catch (final Exception exception) {
                    myColor.setGreen(initialValue);
                    myValueField.setText(String.valueOf(myColor.getGreen()));
                }
            }
        });
