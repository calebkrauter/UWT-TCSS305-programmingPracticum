...
public class RedRowPanel extends JPanel implements PropertyChangeListener {
    ...
    private void addListeners() {
        final int initialValue = myColor.getRed();
        ...
        myDecreaseButton.addActionListener(theEvent -> {
            if (myColor.getRed() == minValue) {
                myDecreaseButton.setEnabled(false);
            } else {
                myColor.adjustRed(-1);
            }
            if (myColor.getRed() < maxValue && !myIncreaseButton.isEnabled()) {
                myIncreaseButton.setEnabled(true);
            }
        });
        myIncreaseButton.addActionListener(theEvent -> {
            if (myColor.getRed() == maxValue) {
                myIncreaseButton.setEnabled(false);
            } else {
                myColor.adjustRed(1);
            }
            if (myColor.getRed() > minValue && !myDecreaseButton.isEnabled()) {
        ...
        myValueSlider.addChangeListener(theEvent -> {
            ...
            if (myColor.getRed() > minValue && !myDecreaseButton.isEnabled()) {
                myDecreaseButton.setEnabled(true);
            } else if (myColor.getRed() == minValue) {
                myDecreaseButton.setEnabled(false);
            }
            if (myColor.getRed() < maxValue && !myIncreaseButton.isEnabled()) {
                myIncreaseButton.setEnabled(true);
            } else if (myColor.getRed() == maxValue) {
                myIncreaseButton.setEnabled(false);
            }
        });
        ...
        myValueField.addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(final ActionEvent theEvent) {
                try {
                    if (myColor.getRed() >= minValue && myColor.getRed() <= maxValue) {
                        myColor.setRed(Integer.valueOf(parseInt(myValueField.getText())));
                    }
                } catch (final Exception exception) {
                    myColor.setRed(initialValue);
                    myValueField.setText(String.valueOf(myColor.getRed()));
                }
            }
        });
